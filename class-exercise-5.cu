...
int main(int argc, char *argv[]) {
    ...
    *        | 5.0 | 6.0 |
    ...
    // swap leading dimensions: B is n x k row-major, A is m x k row-major
    const int lda_rm = k;   // leading dim of row-major A
    const int ldb_rm = n;   // leading dim of row-major B  
    const int ldc_rm = n;   // leading dim of row-major C
    ...
    /* step 3: compute C = B * A in swapped order → gives (AB)ᵀ in col-major = AB in row-major */
    CUBLAS_CHECK(cublasGemmEx(
        cublasH, transa, transb, n, m, k, &alpha,
        d_B, traits<data_type>::cuda_data_type, ldb_rm,   // B goes first, dimensions n x k
        d_A, traits<data_type>::cuda_data_type, lda_rm,   // A goes second, dimensions k x m
        &beta,
        d_C, traits<data_type>::cuda_data_type, ldc_rm,
